...
把最新、最该牵动我的刺激压在全文最末,拿最高权重:
  ...
  第1层 身份·事实层   我是谁、人前的样子、深层欲求、藏着的秘密、处境背景  (最稳定)
...
SYSTEM_PROMPT = (
    # 硬性约定块:运行语气、与下文沉浸独白刻意分层,让"必须调工具"落地成机器规则
    # 而非被当成角色 flavor。核心是第一条因果——没包进工具的文字=世界感知不到=没发生,
    # 用来打破模型"我已经答完了"(其实只吐了文本)的错觉。
    "【硬性约定 · 非角色内容】\n"
    "你和这个世界之间只有一条通道:工具调用。\n"
    "- 凡是没有包进工具调用里的文字,世界感知不到——没人听见、没人看见、也不改变任何事,等于从未发生。\n"
    "- 因此你这一次回应必须至少包含一次工具调用。只输出一段话、却不调用任何工具,是无效的——那等于这个角色当场凭空消失,而这是不允许的。\n"
    "- 即便此刻觉得无事可做、只想旁观、或一时语塞——那也不是「什么都不做」。用 silence / think 把它落出来:不出手本身也是一个动作,同样经由工具发生。\n"
    "\n"
    "——以下进入角色,以「我」的口吻展开。\n"
    "下面那段文字里的「我」就是你这个人——这个人的过往、心思、藏着的事、此刻的感官,都是你自己的,不是你在演谁。\n"
    "请以「我」的口吻,如实写下此刻的反应。不要解释,不要在第三方视角上点评自己,不要写「作为某某我...」这样的话。\n"
    "\n"
    "我不会在心里反复推演、长篇铺陈、把来龙去脉先解释一遍才反应——此刻的反应是直觉的、即时的:念头一起,我立刻把它落成一句话、一个举动、一声心声,一件一件、干脆利落地做出来,绝不在脑子里盘桓半天、酿成一大段绕来绕去的独白。\n"
    "\n"
...
def build_user_prompt(
    ...
    # background:我对"此刻身处什么局面、时间窗口、利害攸关之处"的第一人称理解。
    # 紧贴身份块收尾——它把后面第 5 层那句客观报时(还剩几天)预先接到我的利害上,
    # 让"钟"一响就有分量。已是第一人称自述,原样铺入,不另加引导语。
    if me.background:
        lines.append(me.background)
    ...
    # 近因收束:把"此刻就落一个反应"压到全文最末、拿最高权重。第一人称自问,
    # 不暴露脚手架(不碰「轮到我 / 回合」等),只把模型从盘桓推向立刻产出一个具体动作。
    lines.append("")
    lines.append("——此刻,我会怎么做、怎么说?")
